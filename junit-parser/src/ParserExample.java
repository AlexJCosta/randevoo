...
public class ParserExample {
    ...
    public static void listMethods(String fileName) throws Exception {
        
        class MethodVisitor extends VoidVisitorAdapter<List<String>> {
            @Override
            public void visit(MethodDeclaration n, List<String> names) {
                super.visit(n, names);
                names.add(String.format("%s %s(%s)", n.getType(), n.getName(),n.getParameters()));
            }
        }
        FileInputStream in = new FileInputStream(fileName);
        CompilationUnit compUnit;
        ...
        try {
            ...
            compUnit.accept(new MethodVisitor(), names);
        } finally {
            in.close();
        }
